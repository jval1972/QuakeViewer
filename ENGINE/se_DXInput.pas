...
type
  ...
  protected
    ...
    property StartDelayTime: Integer read FStartDelayTime write
      SetStartDelayTime;
  public
    ...
    property Effects[Index: Integer]: TForceFeedbackEffect read GetEffect;
      default;
    property Index: Integer read GetIndex write SetIndex;
    property Playing: Boolean read GetPlaying;
    property Parent: TForceFeedbackEffect read FParent write SetParent;
    property Name: string read FName write FName;
    property EffectType: TForceFeedbackEffectType read FEffectType write
      SetEffectType;
  ...
  TDXInputState = (isUp, isDown, isLeft, isRight, isButton1, isButton2,
    isButton3,
    isButton4, isButton5, isButton6, isButton7, isButton8, isButton9,
      isButton10, isButton11,
    isButton12, isButton13, isButton14, isButton15, isButton16, isButton17,
      isButton18,
    isButton19, isButton20, isButton21, isButton22, isButton23, isButton24,
      isButton25,
    isButton26, isButton27, isButton28, isButton29, isButton30, isButton31,
      isButton32);
  ...
  published
    property BindInputStates: Boolean read FBindInputStates write
      FBindInputStates;
  ...
  published
    ...
    property DeadZoneX: Integer index DIJOFS_X read GetDeadZone write
      SetDeadZone;
    property DeadZoneY: Integer index DIJOFS_Y read GetDeadZone write
      SetDeadZone;
    property DeadZoneZ: Integer index DIJOFS_Z read GetDeadZone write
      SetDeadZone;
  ...
  public
    ...
    property UseDirectInput: Boolean read FUseDirectInput write
      SetUseDirectInput;
  end;
...
begin
  Result := (FEffectType <> etNone) and ((FObject.FEffect <> nil) or
    (FObject2.FEffect <> nil));
end;
...
procedure TForceFeedbackEffect.CreateEffect;
  ...
  begin
    ...
    if FRoot.FInput.FDevice2 <> nil then
      FRoot.FInput.FDevice2.CreateEffect(GUID, EffectObject.Feff,
        EffectObject.FEffect, nil);
  end;
...
begin
  ...
  if (FRoot.FInput = nil) or (FRoot.FInput.FDevice2 = nil) or
    ...
    (not FRoot.FInput.FForceFeedback) then
    Exit;
...
type
  ...
  published
    ...
    property EffectType: TForceFeedbackEffectType read FEffectType write
      FEffectType;
    property AttackLevel: Integer read FAttackLevel write FAttackLevel default
      0;
    ...
    property Effects: TCollection read FEffects write FEffects stored
      GetStoredEffects;
  end;
...
begin
  if FDXInput.FActiveOnly and (GetForegroundWindow <> FDXInput.FForm.Handle)
    then
    Exit;
...
begin
  Result := Levels[FForceFeedbackDevice and FForceFeedback] or
    Levels2[FDXInput.ActiveOnly];
end;
...
begin
  ...
  begin
    if FDXInput.FDInput.CreateDevice(GUID_SysKeyboard, FDevice, nil) <> DI_OK
      then
      Exit;
...
begin
  ...
  if (not FInitialized) or FDXInput.FActiveOnly and (GetForegroundWindow <>
    FDXInput.FForm.Handle) then
    Exit;
...
begin
  ...
  if (not FEnabled) or (FID < 0) or (csDesigning in FDXInput.ComponentState)
    then
    Exit;
  ...
  try
    try
      ...
      begin
        ...
        FDXInput.FDInput.EnumDevices(DIDEVTYPE_JOYSTICK,
          @TJoystick_EnumJoysticksCallback,
        ...
        {  Device making.  }
        if FDXInput.FDInput.CreateDevice(FDeviceGUID, FDevice, nil) <> DI_OK
          then
          Exit;
        ...
        if FDXInput.FDInput.CreateDevice(GUID_Joystick, FDevice, nil) <> DI_OK
          then
          Exit;
    ...
    begin
      ...
      if FID2 <> -1 then
      begin
        if joyGetDevCaps(FID2, @FJoyCaps, SizeOf(FJoyCaps)) = JOYERR_NOERROR
          then
        begin
...
begin
  ...
  if FDevice <> nil then
  begin
    if SetDWORDProperty(FDevice, DIPROP_DEADZONE, Obj, DIPH_BYOFFSET, Value *
      100) <> DI_OK then
      FDeadZone[Obj] := -1;
  end;
end;
...
begin
  ...
  if FDevice <> nil then
  begin
    if SetRangeProperty(FDevice, DIPROP_RANGE, Obj, DIPH_BYOFFSET, Value) <>
      DI_OK then
      FRange[Obj] := -1;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      JoyInfo.dwFlags := JOY_RETURNX or JOY_RETURNY or JOY_RETURNZ or
        JOY_RETURNPOV or
      ...
      with FJoyCaps do
        Fdijs.lX := ConvertValue(JoyInfo.wXpos, wXmax, wXmin,
          FDeadZone[DIJOFS_X], FRange[DIJOFS_X]);

      with FJoyCaps do
        Fdijs.lY := ConvertValue(JoyInfo.wYpos, wYmax, wYmin,
          FDeadZone[DIJOFS_Y], FRange[DIJOFS_Y]);

      with FJoyCaps do
        Fdijs.lZ := ConvertValue(JoyInfo.wZpos, wZmax, wZmin,
          FDeadZone[DIJOFS_Z], FRange[DIJOFS_Z]);
...
procedure TCustomDXInput.FormWndProc(var Message: TMessage; DefWindowProc:
  TWndMethod);
