...
uses
  Windows, SysUtils, Classes, se_DXConsts;
...
type
  ...
  PDXTextureImage_PaletteEntries = ^TDXTextureImage_PaletteEntries;
  ...
  private
    ...
    FMakeCommonPaletteDone: Boolean;
  ...
  public
    ...
    idx_CommonPalette: TDXTextureImage_PaletteEntries;
    ...
    procedure MakeCommonPalette;
    procedure Clear;
    function GetPixelFast(x, y: Integer): DWORD;
    function GetPixelFast8(x, y: Integer): DWORD; overload;
    function GetPixelFast8(idx: Integer): DWORD; overload;
    function GetPixelFast16(x, y: Integer): DWORD;
    function GetPixelFast24(x, y: Integer): DWORD;
    function GetPixelFast32(x, y: Integer): DWORD;
...
function dxtEncodeChannel(const Channel: TDXTextureImageChannel; c: DWORD):
  DWORD;
begin
  Result := ((c shl Channel._rshift) shr Channel._lshift) and Channel.Mask;
end;

function dxtDecodeChannel(const Channel: TDXTextureImageChannel; c: DWORD):
  DWORD;
begin
...
procedure DXTextureImage_LoadDXTextureImageFunc(Stream: TStream; Image:
  TDXTextureImage); forward;
procedure DXTextureImage_LoadBitmapFunc(Stream: TStream; Image:
  TDXTextureImage); forward;
...
class procedure TDXTextureImage.RegisterLoadFunc(LoadFunc:
...
class procedure TDXTextureImage.UnRegisterLoadFunc(LoadFunc:
  TDXTextureImageLoadFunc);
begin
...
var
  y: Integer;
begin
  SetSize(Source.ImageType, Source.Width, Source.Height, Source.BitCount,
    Source.WidthBytes);

  idx_index := Source.idx_index;
  idx_alpha := Source.idx_alpha;
  idx_palette := Source.idx_palette;
  idx_CommonPalette := Source.idx_CommonPalette;
...
procedure TDXTextureImage.MakeCommonPalette;
var
  pEntry: PPaletteEntry;
  pCommonEntry: PDWord;
  i: integer;
begin
  if not FMakeCommonPaletteDone then
  begin
    pEntry := @idx_palette[0];
    pCommonEntry := @idx_CommonPalette[0];
    for i := 0 to 255 do
    begin
      pCommonEntry^ := (pEntry.peRed shl 16) or
        (pEntry.peGreen shl 8) or
        (pEntry.peBlue);
      inc(pEntry);
      inc(pCommonEntry);
    end;
    FMakeCommonPaletteDone := true;
  end;
end;
...
begin
  ...
  FillChar(idx_CommonPalette, SizeOf(idx_CommonPalette), 0);
...
procedure TDXTextureImage.SetImage(ImageType: TDXTextureImageType; Width,
  Height, BitCount, WidthBytes, NextLine: Integer;
  PBits, TopPBits: Pointer; Size: Integer; AutoFree: Boolean);
begin
  ClearImage;
...
procedure TDXTextureImage.SetSize(ImageType: TDXTextureImageType;
  Width, Height, BitCount, WidthBytes: Integer);
var
  APBits: Pointer;
begin
  ClearImage;
  ...
  SetImage(
    ImageType, Width, Height, BitCount, WidthBytes, WidthBytes, APBits,
    APBits, WidthBytes * Height, True
  );
end;
...
function TDXTextureImage.GetSubGroupImage(GroupTypeID: DWORD; Index: Integer):
  TDXTextureImage;
var
  i, j: Integer;
begin
...
begin
  ...
  begin
    Result :=
      dxtEncodeChannel(idx_index, PaletteIndex(R, G, B)) or
      dxtEncodeChannel(idx_alpha, A);
  end
  else
  begin
    Result :=
      dxtEncodeChannel(rgb_red, R) or
...
begin
  ...
  begin
    ...
    for i := 0 to (1 shl idx_index.BitCount) - 1 do
      ...
      begin
        d2 :=
          Abs((peRed - R)) * Abs((peRed - R)) + Abs((peGreen - G)) *
          Abs((peGreen - G)) + Abs((peBlue - B)) * Abs((peBlue - B));
        if d > d2 then
        begin
          d := d2;
          Result := i;
...
function TDXTextureImage.GetPixelFast(x, y: Integer): DWORD;
begin
  case FBitCount of
    1:
      begin
        if FPackedPixelOrder then
          Result := (PByte(Integer(FTopPBits) + FNextLine * y + x shr 3)^ and
            Mask1[7 - x and 7]) shr Shift1[7 - x and 7]
        else
          Result := (PByte(Integer(FTopPBits) + FNextLine * y + x shr 3)^ and
    ...
    2:
      begin
        if FPackedPixelOrder then
          Result := (PByte(Integer(FTopPBits) + FNextLine * y + x shr 2)^ and
            Mask2[3 - x and 3]) shr Shift2[3 - x and 3]
        else
          Result := (PByte(Integer(FTopPBits) + FNextLine * y + x shr 2)^ and
            Mask2[x and 3]) shr Shift2[x and 3];
      end;
    4:
      begin
        if FPackedPixelOrder then
          Result := (PByte(Integer(FTopPBits) + FNextLine * y + x shr 1)^ and
            Mask4[1 - x and 1]) shr Shift4[1 - x and 1]
        else
          Result := (PByte(Integer(FTopPBits) + FNextLine * y + x shr 1)^ and
...
function TDXTextureImage.GetPixelFast8(x, y: Integer): DWORD;
begin
  Result := PByte(Integer(FTopPBits) + FNextLine * y + x)^;
end;

function TDXTextureImage.GetPixelFast8(idx: Integer): DWORD;
begin
  Result := PByte(Integer(FTopPBits) + idx)^;
end;

function TDXTextureImage.GetPixelFast16(x, y: Integer): DWORD;
begin
  Result := PWord(Integer(FTopPBits) + FNextLine * y + x * 2)^;
end;

function TDXTextureImage.GetPixelFast24(x, y: Integer): DWORD;
begin
  PByte3(@Result)^ := PByte3(Integer(FTopPBits) + FNextLine * y + x * 3)^;
end;

function TDXTextureImage.GetPixelFast32(x, y: Integer): DWORD;
begin
  Result := PDWORD(Integer(FTopPBits) + FNextLine * y + x * 4)^;
end;
...
procedure DXTextureImage_SaveDXTextureImageFunc(Stream: TStream; Image:
  TDXTextureImage); forward;
      ...
      case BlockHeader.ID of
        ...
        DXTextureImageFileBlockID_Image_Format:
          begin
            ...
            if (Header_Image_Format.ImageType <>
              DXTextureImageType_PaletteIndexedColor) and
              (Header_Image_Format.ImageType <> DXTextureImageType_RGBColor) then
              raise EDXTextureImageError.Create(SInvalidDXTFile);

            Image.SetSize(Header_Image_Format.ImageType,
              Header_Image_Format.Width, Header_Image_Format.Height,
              Header_Image_Format.BitCount, Header_Image_Format.Widthbytes);

            if Header_Image_Format.ImageType =
              DXTextureImageType_PaletteIndexedColor then
            begin
              {  INDEX IMAGE  }
              Stream.ReadBuffer(Header_Image_Format_Index,
                SizeOf(Header_Image_Format_Index));

              Image.idx_index :=
                dxtMakeChannel(Header_Image_Format_Index.idx_index_Mask, True);
              Image.idx_alpha :=
                dxtMakeChannel(Header_Image_Format_Index.idx_alpha_Mask, False);

              for i := 0 to 255 do
                Image.idx_palette[i] :=
                  Header_Image_Format_Index.idx_palette[i];
              Image.MakeCommonPalette;
            end
            else if Header_Image_Format.ImageType = DXTextureImageType_RGBColor then
            begin
              {  RGB IMAGE  }
              Stream.ReadBuffer(Header_Image_Format_RGB,
                SizeOf(Header_Image_Format_RGB));

              Image.rgb_red :=
                dxtMakeChannel(Header_Image_Format_RGB.rgb_red_Mask, False);
              Image.rgb_green :=
                dxtMakeChannel(Header_Image_Format_RGB.rgb_green_Mask, False);
              Image.rgb_blue :=
                dxtMakeChannel(Header_Image_Format_RGB.rgb_blue_Mask, False);
              Image.rgb_alpha :=
                dxtMakeChannel(Header_Image_Format_RGB.rgb_alpha_Mask, False);
            end;
          end;
        ...
        DXTextureImageFileBlockID_Image_GroupInfo:
          begin
            ...
            Stream.ReadBuffer(Header_Image_GroupInfo,
              SizeOf(Header_Image_GroupInfo));
        ...
        DXTextureImageFileBlockID_Image_TransparentColor:
          begin
            ...
            Stream.ReadBuffer(Header_Image_TransparentColor,
              SizeOf(Header_Image_TransparentColor));
        ...
        DXTextureImageFileBlockID_Image_PixelData:
          begin
            ...
            for i := 0 to Image.Height - 1 do
              Stream.ReadBuffer(Image.ScanLine[i]^,
                Header_Image_Format.Widthbytes);
          end;
      end;
...
begin
  ...
  try
    ...
    BlockHeader.Size := CurStreamPos - (BlockInfo.StreamPos +
      SizeOf(TDXTextureImageFileBlockHeader));
    FStream.WriteBuffer(BlockHeader, SizeOf(BlockHeader));
  finally
  ...
  begin
    ...
    try
        ...
        try
          ...
          case Image.ImageType of
            DXTextureImageType_PaletteIndexedColor:
              begin
                {  INDEX IMAGE  }
                Header_Image_Format_Index.idx_index_Mask :=
                  Image.idx_index.Mask;
                Header_Image_Format_Index.idx_alpha_Mask :=
                  Image.idx_alpha.Mask;
                for i := 0 to 255 do
                  Header_Image_Format_Index.idx_palette[i] :=
                    Image.idx_palette[i];
                Image.MakeCommonPalette;

                Stream.WriteBuffer(Header_Image_Format_Index,
                  SizeOf(Header_Image_Format_Index));
              end;
            DXTextureImageType_RGBColor:
              begin
                ...
                Stream.WriteBuffer(Header_Image_Format_RGB,
                  SizeOf(Header_Image_Format_RGB));
              end;
          end;
        finally
      ...
      try
        ...
        Stream.WriteBuffer(Header_Image_GroupInfo,
          SizeOf(Header_Image_GroupInfo));
      finally
        BlockHeaderWriter.EndBlock;
      end;
      ...
      try
        ...
        Header_Image_TransparentColor.TransparentColor :=
          Image.TransparentColor;

        Stream.WriteBuffer(Header_Image_TransparentColor,
          SizeOf(Header_Image_TransparentColor));
      finally
        BlockHeaderWriter.EndBlock;
      end;
  ...
  begin
    ...
    begin
      if TopDown then
        ...
        Stream.ReadBuffer(Image.ScanLine[Image.Height - y - 1]^,
          Image.WidthBytes);
    end;
  end;
...
begin
  ...
  case BI.biSize of
    SizeOf(TBitmapCoreHeader):
      begin
        {  OS/2 type  }
        Stream.ReadBuffer(Pointer(Integer(@BC) + 4)^, SizeOf(TBitmapCoreHeader)
          - 4);
    ...
    SizeOf(TBitmapInfoHeader):
      begin
        {  Windows type  }
        Stream.ReadBuffer(Pointer(Integer(@BI) + 4)^, SizeOf(TBitmapInfoHeader)
          - 4);
      end;
  else
    raise EDXTextureImageError.Create(SInvalidDIB);
  ...
  begin
    Image.SetSize(DXTextureImageType_PaletteIndexedColor, BI.biWidth,
      BI.biHeight, BI.biBitCount,
  ...
  begin
    Image.SetSize(DXTextureImageType_RGBColor, BI.biWidth, BI.biHeight,
      BI.biBitCount,
      (((BI.biWidth * BI.biBitCount) + 31) div 32) * 4);
    ...
    j := Image.rgb_red.BitCount + Image.rgb_green.BitCount +
      Image.rgb_blue.BitCount;
    if j < BI.biBitCount then
      Image.rgb_alpha := dxtMakeChannel((1 shl (BI.biBitCount - j) - 1) shl j,
        False);
