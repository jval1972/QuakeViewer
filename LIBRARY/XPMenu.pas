...
History:
========
Nov 30, 2002, By Valavanis Jim
   - Changing Form property type to TWinControl.
Nov 2, 2001, V1.506
...
Creation order:
 Make sure that the creation order of TmmXPMenu comes after any menu or toolbar
...
type
  TOnAcceptComponent =
    procedure(Sender: TObject; Ctrl: TComponent; var Accept: boolean) of object;

  TmmXPMenu = class(TComponent)
  private
    FActive: boolean;
    {Changes MMK FForm to TWinControl}
    FForm: TWinControl;
    {Replaces all TForm -> TWinControl}
    {End Changes}
    FFont: TFont;
    FColor: TColor;
    ...
    FOnAcceptComponent: TOnAcceptComponent;
    ...
    FMakeMenus: boolean;
    FMakeToolbars: boolean;
    FMakeControlBars: boolean;

    procedure SetActive(const Value: boolean);
    procedure SetAutoDetect(const Value: boolean);
    procedure SetMakeMenus(const Value: boolean);
    procedure SetMakeToolbars(const Value: boolean);
    procedure SetMakeControlBars(const Value: boolean);
    procedure SetForm(const Value: TWinControl);
  ...
  protected
    ...
    function AcceptComponent(Ctrl: TComponent): boolean;
    procedure ActivateMenuItem(MenuItem: TMenuItem);
    procedure ItrateMenu(MenuItem: TMenuItem; Enable: boolean);
    procedure Activate(MenuItem: TMenuItem; Enable: boolean);

  public
    constructor Create(AOwner: TComponent); override;
    destructor Destroy; override;
    property Form: TWinControl read FForm write SetForm;
    procedure InitMenueItems(Form: TWinControl; Enable: boolean);
    procedure SubGlassMenu(m: TMenu; Enable: boolean);
  published
    property OnAcceptComponent: TOnAcceptComponent
      read FOnAcceptComponent write FOnAcceptComponent;
    ...
    property MakeMenus: boolean
      read FMakeMenus write SetMakeMenus default true;
    property MakeToolbars: boolean
      read FMakeToolbars write SetMakeToolbars default true;
    property MakeControlBars: boolean
      read FMakeControlBars write SetMakeControlBars default true;
...
procedure Register;
begin
  RegisterComponents('TombViewer Components', [TmmXPMenu]);
end;

{ TXPMenue }

constructor TmmXPMenu.Create(AOwner: TComponent);
begin
...
procedure TmmXPMenu.ActivateMenuItem(MenuItem: TMenuItem);

  procedure Activate(MenuItem: TMenuItem);
  begin
    if addr(MenuItem.OnDrawItem) <> addr(TmmXPMenu.DrawItem) then
    begin
...
procedure TmmXPMenu.SubGlassMenu(m: TMenu; Enable: boolean);
var x: integer;
begin
  if AcceptComponent(m) then
  begin
    for x := 0 to m.Items.Count - 1 do
    begin
      if AcceptComponent(m.Items[x]) then
      begin
        m.OwnerDraw := Enable;
        Activate(m.Items[x], Enable);
        ItrateMenu(m.Items[x], Enable);
      end;
    end;
  end;
end;

procedure TmmXPMenu.Activate(MenuItem: TMenuItem; Enable: boolean);
begin
  if Enable and FMakeMenus then
  begin
    if (not assigned(MenuItem.OnDrawItem)) or (FOverrideOwnerDraw) then
      MenuItem.OnDrawItem := DrawItem;
    if (not assigned(MenuItem.OnMeasureItem)) or (FOverrideOwnerDraw) then
  ...
  begin
    ...
    if addr(MenuItem.OnMeasureItem) = addr(TmmXPMenu.MeasureItem) then
      MenuItem.OnMeasureItem := nil;
  end;
end;

procedure TmmXPMenu.ItrateMenu(MenuItem: TMenuItem; Enable: boolean);
var
  i: integer;
begin
  Activate(MenuItem, Enable);
  for i := 0 to MenuItem.Count - 1 do
    ItrateMenu(MenuItem.Items[i], Enable);
end;

procedure TmmXPMenu.InitMenueItems(Form: TWinControl; Enable: boolean);
...
begin
  ...
  begin
    ...
    begin
      if AcceptComponent(Form.Components[i]) then
      begin
        for x := 0 to TMainMenu(Form.Components[i]).Items.Count - 1 do
        begin
          if AcceptComponent(TMainMenu(Form.Components[i]).Items[x]) then
          begin
            TMainMenu(Form.Components[i]).OwnerDraw := Enable;
            Activate(TMainMenu(Form.Components[i]).Items[x], Enable);
            ItrateMenu(TMainMenu(Form.Components[i]).Items[x], Enable);
          end;
        end;
      end;
    end
    else if Form.Components[i] is TPopupMenu then
    begin
      if AcceptComponent(Form.Components[i]) then
      begin
        for x := 0 to TPopupMenu(Form.Components[i]).Items.Count - 1 do
        begin
          if AcceptComponent(TPopupMenu(Form.Components[i]).Items[x]) then
          begin
            TPopupMenu(FForm.Components[i]).OwnerDraw := Enable;
            Activate(TPopupMenu(Form.Components[i]).Items[x], Enable);
            ItrateMenu(TMainMenu(Form.Components[i]).Items[x], Enable);
          end;
        end;
      end;
    ...
    begin
      ...
      begin
        if FMakeToolBars then
          if AcceptComponent(Form.Components[i]) then
            if not TToolBar(Form.Components[i]).Flat then
              TToolBar(Form.Components[i]).Flat := true;

        if Enable and FMakeToolBars and AcceptComponent(Form.Components[i]) then
        begin
        ...
        begin
          if addr(TToolBar(Form.Components[i]).OnCustomDrawButton) =
            addr(TmmXPMenu.ToolBarDrawButton) then
    ...
    else if Form.Components[i] is TControlBar then
    begin
      if not (csDesigning in ComponentState) then
      begin
        if Enable and FMakeControlBars and AcceptComponent(Form.Components[i]) then
        begin
        ...
        begin
          if addr(TControlBar(Form.Components[i]).OnBandPaint) =
            addr(TmmXPMenu.ControlBarPaint) then
...
procedure TmmXPMenu.DrawItem(Sender: TObject; ACanvas: TCanvas; ARect: TRect;
  Selected: Boolean);
begin
  if FActive then
...
function TmmXPMenu.GetImageExtent(MenuItem: TMenuItem): TPoint;
var
...
procedure TmmXPMenu.MeasureItem(Sender: TObject; ACanvas: TCanvas;
  var Width, Height: Integer);
var
  s: string;
  W, H: integer;
...
procedure TmmXPMenu.MenueDrawItem(Sender: TObject; ACanvas: TCanvas; ARect: TRect;
  Selected: Boolean);
var
  txt: string;
  B: TBitmap;
...
{$IFDEF VER5U}
procedure TmmXPMenu.ToolBarDrawButton(Sender: TToolBar;
  Button: TToolButton; State: TCustomDrawState; var DefaultDraw: Boolean);
...
procedure TmmXPMenu.ControlBarPaint(Sender: TObject; Control: TControl;
  Canvas: TCanvas; var ARect: TRect; var Options: TBandPaintOptions);
var
...
procedure TmmXPMenu.SetGlobalColor(ACanvas: TCanvas);
begin
//-----
...
procedure TmmXPMenu.DrawTopMenuItem(Sender: TObject; ACanvas: TCanvas;
...
procedure TmmXPMenu.DrawCheckedItem(FMenuItem: TMenuItem; Selected,
 HasImgLstBitmap: boolean; ACanvas: TCanvas; CheckedRect: TRect);
var
  X1, X2: integer;
begin
...
procedure TmmXPMenu.DrawTheText(txt, ShortCuttext: string; ACanvas: TCanvas; TextRect: TRect;
...
procedure TmmXPMenu.DrawIcon(Sender: TObject; ACanvas: TCanvas; B: TBitmap;
 IconRect: Trect; Hot, Selected, Enabled, Checked, FTopMenu,
 IsRightToLeft: boolean);
var
  DefColor: TColor;
...
procedure TmmXPMenu.DrawArrow(ACanvas: TCanvas; X, Y: integer);
begin
  ACanvas.MoveTo(X, Y);
...
function TmmXPMenu.TopMenuFontColor(ACanvas: TCanvas; Color: TColor): TColor;
var
  r, g, b, avg: integer;
begin
...
procedure TmmXPMenu.SetMakeMenus(const Value: boolean);
begin
  if FMakeMenus <> Value then
  begin
    FMakeMenus := Value;
    if FActive then
    begin
...
procedure TmmXPMenu.SetMakeToolbars(const Value: boolean);
begin
  if FMakeToolbars <> Value then
  begin
    FMakeToolbars := Value;
    if FActive then
    begin
...
procedure TmmXPMenu.SetMakeControlBars(const Value: boolean);
begin
  if FMakeControlBars <> Value then
  begin
    FMakeControlBars := Value;
    if FActive then
    begin
      InitMenueItems(FForm, false);
      InitMenueItems(FForm, true);
...
procedure TmmXPMenu.SetActive(const Value: boolean);
begin

  FActive := Value;

  InitMenueItems(FForm, false);
  if FActive then InitMenueItems(FForm, true);

  if FForm <> nil then
    Windows.DrawMenuBar(FForm.Handle);
end;

procedure TmmXPMenu.SetAutoDetect(const Value: boolean);
begin
  FAutoDetect := Value;
end;

procedure TmmXPMenu.SetForm(const Value: TWinControl);
var
  Hold: boolean;
begin
  if Value <> FForm then
...
procedure TmmXPMenu.SetFont(const Value: TFont);
begin
  FFont.Assign(Value);
  Windows.DrawMenuBar(FForm.Handle);
...
procedure TmmXPMenu.SetColor(const Value: TColor);
begin
  FColor := Value;
end;

procedure TmmXPMenu.SetIconBackColor(const Value: TColor);
begin
  FIconBackColor := Value;
end;

procedure TmmXPMenu.SetMenuBarColor(const Value: TColor);
begin
  FMenuBarColor := Value;
  Windows.DrawMenuBar(FForm.Handle);
end;

procedure TmmXPMenu.SetCheckedColor(const Value: TColor);
begin
  FCheckedColor := Value;
end;

procedure TmmXPMenu.SetSeparatorColor(const Value: TColor);
begin
  FSeparatorColor := Value;
end;

procedure TmmXPMenu.SetSelectBorderColor(const Value: TColor);
begin
  FSelectBorderColor := Value;
end;

procedure TmmXPMenu.SetSelectColor(const Value: TColor);
begin
  FSelectColor := Value;
end;

procedure TmmXPMenu.SetDisabledColor(const Value: TColor);
begin
  FDisabledColor := Value;
end;

procedure TmmXPMenu.SetSelectFontColor(const Value: TColor);
begin
  FSelectFontColor := Value;
end;

procedure TmmXPMenu.SetIconWidth(const Value: integer);
begin
  FIconWidth := Value;
end;

procedure TmmXPMenu.SetDrawSelect(const Value: boolean);
begin
  FDrawSelect := Value;
end;
...
procedure TmmXPMenu.SetOverrideOwnerDraw(const Value: boolean);
begin
...
procedure TmmXPMenu.SetUseSystemColors(const Value: boolean);
begin
  FUseSystemColors := Value;
  Windows.DrawMenuBar(FForm.Handle);
end;

procedure TmmXPMenu.SetGradient(const Value: boolean);
begin
  FGradient := Value;
end;

procedure TmmXPMenu.SetFlatMenu(const Value: boolean);
begin
  FFlatMenu := Value;
end;
...
procedure TmmXPMenu.DrawGradient(ACanvas: TCanvas; ARect: TRect;
 IsRightToLeft: boolean);
var
  i: integer;
  v: integer;
...
procedure TmmXPMenu.DrawWindowBorder(hWnd: HWND; IsRightToLeft: boolean);
var
  WRect, CRect: TRect;
  dCanvas: TCanvas;
begin
...
procedure TmmXPMenu.Notification(AComponent: TComponent;
...
function TmmXPMenu.AcceptComponent(Ctrl: TComponent): boolean;
begin
  result := true;
