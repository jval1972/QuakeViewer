...
uses
  Windows, Graphics, Controls, Classes, Forms, SysUtils, Consts, Math, Dialogs;
...
procedure CreateColorArray(Color1, Color2: TColor; Steps: Integer; var Colors:
  TColorsArray);
procedure GradientFillColors(Canvas: TCanvas; Colors: TColorsArray; R: TRect);
procedure GradientFill(Canvas: TCanvas; FBeginColor, FEndColor: TColor; R:
  TRect; FillStyle: TGradientFill = gfLinear);
...
procedure DrawGrayText(Canvas: TCanvas; Text: string; var R: TRect; Flags:
  Integer);
procedure RotateText(Canvas: TCanvas; Text: string; var R: TRect; Angle:
  Integer);
...
function ParseSection(ParseLine: string; ParseNum: Integer; ParseSep: Char):
  string;
...
function StripString(StrData: string; StripChars: TrmCharSet; ReplaceChar:
  char): string;
function rmDateTimeToStr(x: TDateTime): string;
function MaskStrCmp(mask, s: string): boolean;
function Soundex(InStr: string; StandardResult: boolean = True; Precision:
  integer = 4): string;
...
{ OS }
type
  TWinOSVersion = (wosWin3x, wosWin95, wosWin98, wosWinNT3x, wosWinNT4x,
    wosWinNT2k, wosWinNTX, wosUnknown);
  TrmHkey = (HKEY_CLASSES_ROOT, HKEY_CURRENT_USER, HKEY_LOCAL_MACHINE,
    HKEY_USERS,
...
uses
  shellapi, ShlObj, WinSock, ActiveX;
...
begin
  try
    Result := round(RectHeight(Rect) / Sin(arcTan(RectHeight(Rect) /
      RectWidth(Rect)))) + 2
  except
    Result := 0;
  end;
...
begin
  ...
  begin { iterate through the color bands }
    ...
    try
      PatBlt(Canvas.handle, ColorBand.Left, ColorBand.Top, ColorBand.Right -
        ColorBand.Left, wHeight, PATCOPY);
    finally
...
begin
  ...
  begin { iterate through the color bands }
    ...
    begin
      ...
      begin
        wRed := wBeginRGBValue[0] +
          MulDiv(wIndex, wRGBDifference[0], (wSteps shr 1) - 1);
        wGreen := wBeginRGBValue[1] +
          MulDiv(wIndex, wRGBDifference[1], (wSteps shr 1) - 1);
        wBlue := wBeginRGBValue[2] +
          MulDiv(wIndex, wRGBDifference[2], (wSteps shr 1) - 1);
      end
      else
      begin
        wRed := wBeginRGBValue[0] +
          MulDiv(((wSteps - 1) - wIndex), wRGBDifference[0], (wSteps shr 1));
        wGreen := wBeginRGBValue[1] +
          MulDiv(((wSteps - 1) - wIndex), wRGBDifference[1], (wSteps shr 1));
        wBlue := wBeginRGBValue[2] +
          MulDiv(((wSteps - 1) - wIndex), wRGBDifference[2], (wSteps shr 1));
      end;
    end;
      ...
      case FillStyle of
        gfLinear:
          begin
            ...
            PatBlt(Canvas.handle, wColorBand.Left, wColorBand.Top,
              wColorBand.Right - wColorBand.Left, wHeight, PATCOPY);
        ...
        gfRectangle:
          begin
            ...
            try
              Rectangle(Canvas.Handle, wColorBand.left, wColorBand.Top,
                wColorBand.right, wColorBand.Bottom);
        ...
        gfRoundRect:
          begin
            ...
            wOldPen := SelectObject(Canvas.handle, wPen);
            try
              RoundRect(Canvas.Handle, wColorBand.left, wColorBand.Top,
                wColorBand.right, wColorBand.Bottom, 30, 30);
              InflateRect(wColorBand, -1, -1);
            finally
              SelectObject(Canvas.handle, wOldPen);
              DeleteObject(wPen);
        ...
        gfOval:
          begin
            ...
            wOldPen := SelectObject(Canvas.handle, wPen);
            try
              Ellipse(Canvas.Handle, wColorBand.left, wColorBand.Top,
                wColorBand.right, wColorBand.Bottom);
              InflateRect(wColorBand, -1, -1);
            finally
              SelectObject(Canvas.handle, wOldPen);
        ...
        gfRadial:
          begin
            ...
            try
              ...
              pie(Canvas.handle, 0, 0, wWidth, wHeight, Round(((wWidth shr 1)) *
                (1 - Cos(wAngle + wIncAngle))), Round(((wheight shr 1)) * (1 -
                Sin(wAngle + wIncAngle))), wlastx, wlasty);
...
begin
  ...
  BitmapRotated.canvas.fillrect(rect(0, 0, bitmaprotated.width,
...
begin
  ...
  Bigger := RectDiameter(rect(0, 0, canvas.textwidth(text),
      ...
      try
        ...
        try
          ...
          TextOut(Canvas.Handle,
            r.Left + (bigger shr 1), r.Top + (bigger shr 1), pchar(text), Length(text));
        finally
...
begin
  ...
  begin
    ...
    begin
      crc := ((crc shr 8) and $FFFFFF) xor fcrctable[(crc xor buffer[checked])
        and $FF];
      inc(checked);
    end;
  end;
...
begin
  ...
  begin
    crc := ((crc shr 8) and $FFFFFF) xor fcrctable[(crc xor byte(data[index]))
      and $FF];
    inc(index);
  end;
...
begin
  ...
  Result := inttostr(y) + inttostr(mth) + inttostr(d) + inttostr(h) + inttostr(m)
    + inttostr(s) + inttostr(ms);
end;
...
const
  CLSID_ShellLink: TGUID = (D1: $00021401; D2: $0; D3: $0; D4: ($C0, $0, $0, $0,
    $0, $0, $0, $46));
  IID_IShellLink: TGUID = (D1: $000214EE; D2: $0000; D3: $0000; D4: ($C0, $00,
    $00, $00, $00, $00, $00, $46));
  IID_IPersistFile: TGUID = (D1: $0000010B; D2: $0000; D3: $0000; D4: ($C0, $00,
    $00, $00, $00, $00, $00, $46));

  ShowCommands: array[TWindowState] of Integer = (SW_SHOWNORMAL,
    SW_SHOWMINNOACTIVE, SW_SHOWMAXIMIZED);
...
begin
  ...
  SHGetFileInfo(pchar(wname), wattr, sfi, sizeof(sfi), SHGFI_USEFILEATTRIBUTES or
    SHGFI_TYPENAME);
...
begin
  ...
  try
    ...
    if smallImage then
      ShellImages.handle := SHGetFileInfo(pchar(wname), wattr, sfi, sizeof(sfi),
...
begin
  ...
  if smallImage then
    SHGetFileInfo(pchar(wname), wattr, sfi, sizeof(sfi), SHGFI_USEFILEATTRIBUTES
      or SHGFI_SYSICONINDEX or SHGFI_SMALLICON)
  else
    SHGetFileInfo(pchar(wname), wattr, sfi, sizeof(sfi), SHGFI_USEFILEATTRIBUTES
      or SHGFI_SYSICONINDEX or SHGFI_LARGEICON);
...
begin
  ...
  if smallImage then
    AImageList.handle := SHGetFileInfo(pchar(wname), 0, sfi, sizeof(sfi),
    ...
    VER_PLATFORM_WIN32_WINDOWS:
      begin
        if ((verinfo.dwMajorVersion > 4) or
          ((verinfo.dwMajorVersion = 4) and (verinfo.dwMinorVersion >= 10)))
            then
          Result := wosWin98
        else
          Result := wosWin95;
...
begin
  if low > high then
    raise ERangeError.Create(inttostr(low) + ' is not less than or equal to ' +
      inttostr(high));
